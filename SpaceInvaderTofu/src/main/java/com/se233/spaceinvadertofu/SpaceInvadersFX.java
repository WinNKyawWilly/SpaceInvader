...
public class SpaceInvadersFX extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        primaryStage.setOnCloseRequest(e -> {
            timer.stop();
            HelloApplication.GAME_SET = false;
            primaryStage.close();
        });
    ...
    private void spawnEnemies() {
        for (int y = 80, i = 0; y < APP_HEIGHT / 2 + SPACE && i < 5; y += SPACE, i++) {
            for (int x = APP_WIDTH/3 - (SPACE*3), j = 0; x < 660 && j < 13; x += SPACE, j++) {
                if (y < 90) {
                    enemies[i][j] = spawnAlien(x, y, "assets/images/small_invader_a.png");
                    gc.drawImage(enemies[i][j].getImage(), x, y);
                } else if (y < 200) {
                    enemies[i][j] = spawnAlien(x, y, "assets/images/medium_invader_a.png");
                    gc.drawImage(enemies[i][j].getImage(), x, y);
                } else {
                    enemies[i][j] = spawnAlien(x, y, "assets/images/large_invader_a.png");
                    gc.drawImage(enemies[i][j].getImage(), x, y);
                }
                totalEnemies++;
            }
    ...
    private void setMovedEnemies() {
        for (int y = 80, i = 0; y < APP_HEIGHT / 2 + SPACE && i < 5; y += SPACE, i++) {
            for (int x = APP_WIDTH/3 - (SPACE*3), j = 0; x < 660 && j < 13; x += SPACE, j++) {
                if (y < 90) {
                    enemiesMoved[i][j] = spawnAlien(x, y, "assets/images/small_invader_b.png");
                } else if (y < 200) {
                    enemiesMoved[i][j] = spawnAlien(x, y, "assets/images/medium_invader_b.png");
                } else {
                    enemiesMoved[i][j] = spawnAlien(x, y, "assets/images/large_invader_b.png");
                }
            }
        }
    }
